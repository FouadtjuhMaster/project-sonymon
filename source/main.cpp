	...
	{  
	   ...
	   {	
		  ...
		  if(menuChoice == 1){
		    ...
		    if(osl_keys->pressed.cross || osl_keys->pressed.start) {selected = 1; oslPlaySound(select, 2); break;}                      
		  }
        
          else if(menuChoice == 2){
			placeSelector(rec_selector, 3, 58);
			if(osl_keys->pressed.cross || osl_keys->pressed.start){oslPlaySound(select, 2); askContinue();}
		  }	
		
		  else if(menuChoice == 3){
			placeSelector(rec_selector, 3, 110);
			if(osl_keys->pressed.cross || osl_keys->pressed.start){oslPlaySound(select, 2);  askOptions();}
		  }
...
//this will allow the player to resume an old game
void askContinue( void )
{
     ...
     while (1)
	   {	
		  ...
		  if(osl_keys->pressed.circle) {oslPlaySound(select, 2); break;}
		  else if(osl_keys->pressed.down){continueChoice++; oslPlaySound(select, 2);}
		  else if(osl_keys->pressed.up){continueChoice--; oslPlaySound(select, 2);}
		  
		  if(continueChoice > 2) continueChoice = 1;
		  ...
		  if(continueChoice == 1)
          {
		    ...
		    if(osl_keys->pressed.cross || osl_keys->pressed.start)
              {oslPlaySound(select, 2); Sonymon(playerName, 1); break;}                      
		  }
          ...
          else if(continueChoice == 2)
          {
			placeSelector(rec_selector, 3, 58);
			if(osl_keys->pressed.cross || osl_keys->pressed.start)
			     {oslPlaySound(select, 2); SonymonFreeRoam(playerName, 1); break;}
		  }	
		
		  oslStartDrawing();
		  oslDrawImage(menu_background3);
          oslDrawImage(rec_selector);
		  oslEndDrawing();
		  oslSyncFrame();		
      }

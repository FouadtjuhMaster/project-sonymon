     ...
     while(1)
     {
               ...
               else if(pChoice == 6)
               {
			      ...
			      if(osl_keys->pressed.cross || osl_keys->pressed.start)
			        SAVEGAME(sonymon1.id, sonymon1.level, sonymon2.id, sonymon2.level, sonymon3.id, sonymon3.level);
               }
...
void MENU::OPEN_PLAYER(const char * playerName)
{
     ...
     while(1)
     {
             ...
             oslDrawString(5, 5, "Player information:");
             oslPrintf_xy(5, 35, "Player name: %s", playerName);
             ...
             if(sonymon_num == 1) oslPrintf_xy(5, 65, "Current sonymon equipped: %s", sonymon1.name);
             else if(sonymon_num == 2) oslPrintf_xy(5, 75, "Current sonymon equipped: %s", sonymon2.name);
             else if(sonymon_num == 3) oslPrintf_xy(5, 85, "Current sonymon equipped: %s", sonymon3.name);
...
void MENU::SHOWSTATS(const int slot, const char * playerName = "default")
{
    ...
    int id = 1;
    if(slot == 1) id = sonymon1.id;
    else if(slot == 2) id = sonymon2.id;
    else if(slot == 3) id = sonymon3.id;
    ...
    for(;;)
    {
        ...
        {   
            ...
            oslSetTextColor(RED);
            oslPrintf_xy(5, 20, "%s Lv:%d", sonymon1.name, sonymon1.level);
            oslSetTextColor(WHITE);
            ...
            if(showStats)
            {
               ...
               oslPrintf_xy(380, 130, "%d/%d", sonymon1.health, sonymon1.maxHealth);
               oslPrintf_xy(350, 145, "Exp: ");
               oslPrintf_xy(380, 145, "%d/%d", sonymon1.xp, sonymon1.max_xp);
            ...
            {
                oslSetTextColor(WHITE);
                oslPrintf_xy(200, 170, "%s", des1);
                oslPrintf_xy(200, 180, "%s", des2);
                oslPrintf_xy(200, 190, "%s", des3);
                oslPrintf_xy(200, 200, "%s", des4);
                oslPrintf_xy(200, 210, "%s", des5);
           }
        }
        //draw sonymon 2 stats
        else if(slot == 2){
            oslPrintf_xy(TempImage->stretchX + 20, TempImage->stretchY - 10, "%s", sonymon2.name);
        }
        
        //draw sonymon 3 stats
        else if(slot == 3){
            oslPrintf_xy(TempImage->stretchX + 20, TempImage->stretchY - 10, "%s", sonymon3.name);
        }
...
void MENU::OPEN_PARTY(bool inBattle, const char * playerName = "default")
{
     ...
     if(totalSonymon == 1){
                       stats.setPartyImage(1, sonymon1.id);
     }
     else if(totalSonymon == 2){
                       stats.setPartyImage(1, sonymon1.id);
                       stats.setPartyImage(2, sonymon2.id);
     }
     else if(totalSonymon == 3){
                       stats.setPartyImage(1, sonymon1.id);
                       stats.setPartyImage(2, sonymon2.id);
                       stats.setPartyImage(3, sonymon3.id);
     }
     ...
     while(1)
     {
        ...
        if(totalSonymon == 1){
                          oslDrawImage(temp1);
                          oslPrintf_xy(33, 68, "%s Lv %d", sonymon1.name, sonymon1.level);
                          oslPrintf_xy(33, 83, "HP: ");
                          
                          long countHealth = sonymon1.maxHealth;
                          long count2Health = sonymon1.maxHealth / 2;
                          
                          countHealth /= 2; 
                              if(countHealth >= sonymon1.health) oslSetTextColor(ORANGE);
                              
                          countHealth = sonymon1.maxHealth;
                          
                          countHealth /= 4;
                              if(countHealth >= sonymon1.health) oslSetTextColor(RED);
                              
                          countHealth = sonymon1.maxHealth;
                              if(sonymon1.health <= countHealth && sonymon1.health > count2Health) oslSetTextColor(GREEN);
                          
                          oslPrintf_xy(63, 83, "%d/%d", sonymon1.health, sonymon1.maxHealth);
